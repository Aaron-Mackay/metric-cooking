...
const ingredients = {
  ...
  'flour': [/\b(all[- ]purpose |sifted |unbleached |bread |white |self[- ]raising )*flour/, 125 / cup_ml], // ~20081~
...
const thekitchn = typeof document !== 'undefined' && location.hostname.includes('thekitchn');
...
function walk(node) {
  switch (node.nodeType) {
    ...
    case Node.DOCUMENT_FRAGMENT_NODE:
      ...
      } else if (thekitchn && node.className === 'Recipe__ingredient') {
        let quantityNode, itemNameNode, unitNode;
        for (let child = node.firstChild; child; child = child.nextSibling) {
          if (child.className === 'Recipe__ingredientQuantity') {
            quantityNode = child;
          } else if (child.className === 'Recipe__ingredientText') {
            for (const grandchild of child.childNodes)
              if (grandchild.className === 'Recipe__ingredientMeasurementUnit') {
                unitNode = grandchild;
              } else if (grandchild.className === 'Recipe__ingredientName') {
                itemNameNode = grandchild;
              }
          }
        }
        if (quantityNode && itemNameNode && unitNode) {
          let text = quantityNode.textContent + unitNode.textContent + itemNameNode.textContent;
          const modified = re.replace(text, replaceUnits);
          if (modified != text) {
            quantityNode.innerText = modified;
            unitNode.remove();
            itemNameNode.remove();
          }
        }
